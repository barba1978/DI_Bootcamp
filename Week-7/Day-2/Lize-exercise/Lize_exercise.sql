-- Database: Lise_exercise

-- DROP DATABASE IF EXISTS "Lise_exercise";
/*
CREATE DATABASE "Lise_exercise"
    WITH 
    OWNER = postgres
    ENCODING = 'UTF8'
    LC_COLLATE = 'C'
   ...
   3. Department_code in the Employee table is the FK of the Department table (reference the DEPTCODE column of the Department table)
...
SELECT EmpFName ,job, from employee???
...
ON employee.column_name = department.column_name;

--7.  Display EMPFNAME and "TOTAL SALARY" for each employee (commission and salary)
SELECT EMPFNAME, SUM(SALARY)FROM EMPLOY  ????
